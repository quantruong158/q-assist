  ...
  handle: (text: string): string => {
    ...
    outer: while (true) {
      ...
      if (
        ...
        // Only strip ``` that follows a newline (or is the entire text) — those are fence openers.
        // A ``` not at the start of a line could be inside text (rare but possible).
      ...
      // Inline code ` (one or more, but not ```)
      if (last === 96) {
        // Count the run of backticks at the tail
        let btCount = 0;
...
const closeFormattingMarkers: RemendHandler = {
  ...
  handle: (text: string): string => {
    ...
    while (i < len) {
      ...
      // Fenced code block ``` — skip its content entirely (markers inside are literal)
      if (
        cc === 96 &&
        i + 2 < len &&
      ...
      // Inline code ` — toggle inInlineCode; other markers are ignored while inside
      if (cc === 96) {
        inInlineCode = !inInlineCode;
        if (!inInlineCode) {
          // Closing backtick: pop the matching opener from the stack
          const topIdx = stack.findLastIndex((s) => s.marker === '`');
      ...
      // While inside inline code, all characters are literal
      if (inInlineCode) {
        i++;
        continue;
      }
      ...
      // Italic * — opener only when NOT followed by whitespace (avoids list items: `* item`)
      if (cc === 42) {
        const topIdx = stack.findLastIndex((s) => s.marker === '*');
        if (topIdx >= 0) {
      ...
      // Italic _ — opener only when NOT followed by whitespace
      if (cc === 95) {
        const topIdx = stack.findLastIndex((s) => s.marker === '_');
        if (topIdx >= 0) {
          stack.splice(topIdx, 1);
        } else {
          const prevIsWs = i === 0 || isWhitespace(text[i - 1]);
