...
import { CdkMenu, MENU_TRIGGER, type CdkMenuTriggerBase } from '@angular/cdk/menu';
...
export class HlmDropdownMenu {
  ...
  private readonly _trigger = inject(MENU_TRIGGER, { optional: true }) as
    | CdkMenuTriggerBase
    | undefined;
  ...
  constructor() {
    ...
    if (trigger) {
      trigger.closed.pipe(takeUntilDestroyed()).subscribe(() => this._state.set('closed'));
    } else {
